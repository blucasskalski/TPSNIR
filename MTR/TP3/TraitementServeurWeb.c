...
void HandleTCPClient(int client_socket){
    ...
    if( (ret == 0) && S_ISREG(statFile.st_mode) ){
		...
		if( f == NULL ){
			...
			if (debug) printf ("fichier ouvert\n");
		}
    }else {
	...
	if( fin ) printf("arrêt prématuré demandé\n");
    if(j == 0){
